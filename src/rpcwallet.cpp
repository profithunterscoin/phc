...
static void accountingDeprecationCheck()
{
    ...
    {
        throw runtime_error("Accounting API is deprecated and will be removed in future.\n"
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CPHCcoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
    {
        throw runtime_error("getnewaddress ( \"account\" )\n"
                            "\nReturns a new PHC address for receiving payments.\n"
                            "If 'account' is specified (recommended), it is added to the address book \n"
                            "so payments received with the address will be credited to 'account'.\n"
                            "\nArguments:\n"
                            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
                            "\nResult:\n"
                            "\"PHCaddress\"    (string) The new PHC address\n"
                            "\nExamples:\n"
    ...
    return CPHCcoinAddress(keyID).ToString();
}


CPHCcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CPHCcoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
    {
        throw runtime_error("getaccountaddress \"account\"\n"
                            "\nReturns the current PHC address for receiving payments to this account.\n"
                            "\nArguments:\n"
                            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
                            "\nResult:\n"
                            "\"PHCaddress\"   (string) The account PHC address\n"
                            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("setaccount \"PHCaddress\" \"account\"\n"
                            "\nSets the account associated with the given address.\n"
                            "\nArguments:\n"
                            "1. \"PHCaddress\"  (string, required) The PHC address to be associated with an account.\n"
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
    {
        throw runtime_error("getaccount \"PHCaddress\"\n"
                            "\nReturns the account associated with the given address.\n"
                            "\nArguments:\n"
                            "1. \"PHCaddress\"  (string, required) The PHC address for account lookup.\n"
                            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
    {
        throw runtime_error("getaddressesbyaccount \"account\"\n"
                            ...
                            "  \"PHCaddress\"  (string) a PHC address associated with the given account\n"
                            "  ,...\n"
                            "]\n"
                            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CPHCcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CPHCcoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("sendtoaddress \"PHCaddress\" amount ( \"comment\" \"comment-to\" )\n"
                            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
                            + HelpRequiringPassphrase() +
                            "\nArguments:\n"
                            "1. \"PHCaddress\"  (string, required) The PHC address to send to.\n"
                            "2. \"amount\"      (numeric, required) The amount in PHC to send. eg 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
    {
        throw runtime_error("listaddressgroupings\n"
                            ...
                            "  [\n"
                            "    [\n"
                            "      \"PHCaddress\",     (string) The PHC address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CPHCcoinAddress(address).ToString());
            ...
            // Global Namespace Start
            {
                ...
                if (pwalletMain->mapAddressBook.find(CPHCcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                {
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CPHCcoinAddress(address).Get())->second);
                }
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("signmessage \"PHCaddress\" \"message\"\n"
                            "\nSign a message with the private key of an address"
                            + HelpRequiringPassphrase() + "\n"
                            "\nArguments:\n"
                            "1. \"PHCaddress\"  (string, required) The PHC address to use for the private key.\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
    {
        throw runtime_error("getreceivedbyaddress \"PHCaddress\" ( minconf )\n"
                            "\nReturns the total amount received by the given PHCaddress in transactions with at least minconf confirmations.\n"
                            "\nArguments:\n"
                            "1. \"PHCaddress\"  (string, required) The PHC address for transactions.\n"
                            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
                            "\nResult:\n"
                            "amount   (numeric) The total amount in PHC received at this address.\n"
                            "\nExamples:\n"
    ...
    // Bitcoin address
    CPHCcoinAddress address = CPHCcoinAddress(params[0].get_str());
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
    {
        throw runtime_error("getreceivedbyaccount \"account\" ( minconf )\n"
                            "\nReturns the total amount received by addresses with <account> in transactions with at least [minconf] confirmations.\n"
                            "\nArguments:\n"
                            "1. \"account\"      (string, required) The selected account, may be the default account using \"\".\n"
                            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
                            "\nResult:\n"
                            "amount              (numeric) The total amount in PHC received for this account.\n"
                            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("move \"fromaccount\" \"toaccount\" amount ( minconf \"comment\" )\n"
                            ...
                            "\nExamples:\n"
                            "\nMove 0.01 PHC from the default account to the account named tabby\n"
                            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
                            "\nMove 0.01 PHC timotei to akiko with a comment and funds have 10 confirmations\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("sendfrom \"fromaccount\" \"toPHCaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
                            "\nSent an amount from an account to a PHC address.\n"
                            "The amount is a real and is rounded to the nearest 0.00000001."
                            + HelpRequiringPassphrase() + "\n"
                            "\nArguments:\n"
                            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                            "2. \"toPHCaddress\"  (string, required) The PHC address to send funds to.\n"
                            "3. amount                (numeric, required) The amount in PHC. (transaction fee is added on top).\n"
                            ...
                            "\"transactionid\"        (string) The transaction id.\n"
                            "\nExamples:\n"
                            "\nSend 0.01 PHC from the default account to the address, must have at least 1 confirmation\n"
    ...
    CPHCcoinAddress address(params[1].get_str());
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
    {
        throw runtime_error("sendmany \"fromaccount\" {\"address\":amount,...} ( minconf \"comment\" )\n"
                            ...
                            "    {\n"
                            "      \"address\":amount   (numeric) The PHC address is the key, the numeric amount in PHC is the value\n"
                            "      ,...\n"
                            "    }\n"
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
        {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid PHC address: ")+s.name_);
        }
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a PHC address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of PHC addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) PHC address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"PHCaddress\"  (string) A PHC address associated with the keys.\n"
    ...
    return CPHCcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CPHCcoinAddress, tallyitem> mapTally;
    ...
    BOOST_FOREACH(const PAIRTYPE(CPHCcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CPHCcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
    {
        throw runtime_error("listreceivedbyaddress ( minconf includeempty includeWatchonly)\n"
                            ...
                            "    \"amount\" : x.xxx,                  (numeric) The total amount in PHC received by the address\n"
...
Value listreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
    {
        throw runtime_error("listreceivedbyaccount ( minconf includeempty includeWatchonly)\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("listtransactions ( \"account\" count from includeWatchonly)\n"
                            ...
                            "    \"address\":\"PHCaddress\",    (string) The PHC address of the transaction. Not present for \n"
                            "                                                move transactions (category = move).\n"
                            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
                            "                                                transaction between accounts, and not associated with an address,\n"
                            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
                            "                                                associated with an address, transaction id and block details\n"
                            "    \"amount\": x.xxx,          (numeric) The amount in PHC. This is negative for the 'send' category, and for the\n"
...
Value listaccounts(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
    {
        throw runtime_error("listaccounts ( minconf includeWatchonly)\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
    {
       throw runtime_error("listsinceblock ( \"blockhash\" target-confirmations includeWatchonly)\n"
                            ...
                            "    \"address\":\"PHCaddress\",    (string) The PHC address of the transaction. Not present for move transactions (category = move).\n"
                            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
                            "    \"amount\": x.xxx,          (numeric) The amount in PHC. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
    {
        throw runtime_error("gettransaction \"txid\" ( includeWatchonly )\n"
                            ...
                            "      \"address\" : \"PHCaddress\",   (string) The PHC address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("walletpassphrase \"passphrase\" timeout ( anonymizeonly )\n"
                            "\nStores the wallet decryption key in memory for 'timeout' seconds.\n"
                            "This is needed prior to performing transactions related to private keys such as sending PHC\n"
                            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
    {
        throw runtime_error("encryptwallet \"passphrase\"\n"
                            ...
                            "\nNow set the passphrase to use the wallet, such as for signing or sending PHC\n"
                            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
                            "\nNow we can so something like sign\n"
                            + HelpExampleCli("signmessage", "\"PHCaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; PHC server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
    {
        throw runtime_error("settxfee amount\n"
                            ...
                            "\nArguments:\n"
                            "1. amount         (numeric, required) The transaction fee in PHC/kB rounded to the nearest 0.00000001\n"
                            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
    {
        throw runtime_error("getnewstealthaddress [label]\n"
                            "Returns a new PHC stealth address for receiving payments anonymously.  ");
    }
...
Value liststealthaddresses(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
    {
        throw runtime_error("liststealthaddresses [show_secrets=0]\n"
...
Value importstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2)
    {
        throw runtime_error("importstealthaddress <scan_secret> <spend_secret> [label]\n"
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        throw runtime_error("sendtostealthaddress <stealth_address> <amount> [comment] [comment-to] [narration]\n"
    ...
    {
        result.push_back(Pair("result",         "Invalid PHC stealth address."));
...
Value scanforalltxns(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
    {
        throw runtime_error("scanforalltxns [fromHeight]\n"
