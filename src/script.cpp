...
const char* ScriptErrorString(const ScriptError serror)
{
    switch (serror)
    {
        ...
        case SCRIPT_ERR_PUBKEYTYPE: case SCRIPT_ERR_UNKNOWN_ERROR: case SCRIPT_ERR_ERROR_COUNT: default:
        {
            break;
        }
    }
...
CBigNum CastToBigNum(const valtype& vch, const size_t nMaxNumSize = nDefaultMaxNumSize)
{
    ...
    {
        throw runtime_error("ERROR - CastToBigNum() : overflow");
    }
...
static inline void popstack(vector<valtype>& stack)
{
    if (stack.empty())
    {
        throw runtime_error("ERROR - popstack() : stack empty");
    }
        ...
        while (pc < pend)
        {
                ...
                && opcode <= OP_ENDIF))
            {
                switch (opcode)
                {
                    ...
                    // Push value
                    //
                    case OP_1NEGATE: case OP_1: case OP_2: case OP_3: case OP_4: case OP_5: case OP_6: case OP_7:
                    case OP_8: case OP_9: case OP_10: case OP_11: case OP_12: case OP_13: case OP_14: case OP_15:
                    case OP_16:
                    {
                        // ( -- value)
                    ...
                    // Numeric
                    //
                    case OP_1ADD: case OP_1SUB: case OP_2MUL: case OP_2DIV: case OP_NEGATE:
                    case OP_ABS: case OP_NOT: case OP_0NOTEQUAL:
                    {
                        // (in -- out)
                        if (stack.size() < 1)
                        {
                        ...
                        switch (opcode)
                        {
                            ...
                            default:
                            {
                                if (fDebug)
                                {
                                    ...
                                    LogPrint("script", "%s : ERROR - Invalid op code \n", __FUNCTION__);
                                }

                                return error("ERROR - Invalid op code");
                    ...
                    case OP_ADD: case OP_SUB: case OP_MUL: case OP_DIV: case OP_MOD: case OP_LSHIFT: case OP_RSHIFT:
                    case OP_BOOLAND: case OP_BOOLOR: case OP_NUMEQUAL: case OP_NUMEQUALVERIFY: case OP_NUMNOTEQUAL:
                    case OP_LESSTHAN: case OP_GREATERTHAN: case OP_LESSTHANOREQUAL: case OP_GREATERTHANOREQUAL:
...
// OPENSSL 1.1+
                    ...
                    case OP_CHECKSIG: case OP_CHECKSIGVERIFY:
                    {
                        ...
                        bool fSuccess = CheckSignatureEncoding(vchSig) && CheckPubKeyEncoding(vchPubKey) && CheckSig(vchSig, vchPubKey, scriptCode, txTo, nIn, nHashType, flags);
                    ...
                    case OP_CHECKMULTISIG: case OP_CHECKMULTISIGVERIFY:
                    {
                        ...
                        while (fSuccess && nSigsCount > 0)
                        {
                            ...
                            // Check signature
                            bool fOk = CheckSignatureEncoding(vchSig) && CheckPubKeyEncoding(vchPubKey) && CheckSig(vchSig, vchPubKey, scriptCode, txTo, nIn, nHashType, flags);
                        ...
                        if ((flags & SCRIPT_VERIFY_NULLDUMMY)
                            && stacktop(-1).size())
                        {
                            return error("ERROR - CHECKMULTISIG dummy argument not null");
                        }
...
bool static CheckPubKeyEncoding(const valtype &vchSig, unsigned int flags, ScriptError* serror)
{
    if ((flags & SCRIPT_VERIFY_STRICTENC) != 0
        && !IsCompressedOrUncompressedPubKey(vchSig))
    {
        ...
        while (pc < pend)
        {
                ...
                && opcode <= OP_ENDIF))
            {
                switch (opcode)
                {
                    ...
                    // Push value
                    //
                    case OP_1NEGATE: case OP_1: case OP_2: case OP_3: case OP_4: case OP_5: case OP_6:
                    case OP_7: case OP_8: case OP_9: case OP_10: case OP_11: case OP_12: case OP_13:
                    case OP_14: case OP_15: case OP_16:
                    {
                        // ( -- value)
                    ...
                    case OP_NUMNOTEQUAL: case OP_LESSTHAN: case OP_GREATERTHAN: case OP_LESSTHANOREQUAL: case OP_GREATERTHANOREQUAL:
                    case OP_MIN: case OP_MAX:
                    {
                        // (x1 x2 -- out)
                        if (stack.size() < 2)
...
uint256 SignatureHash(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    if (nIn >= txTo.vin.size())
    {
        if (fDebug)
        {
            LogPrint("script", "%s : ERROR - SignatureHash() : nIn=%d out of range \n", __FUNCTION__, nIn);
        }
    ...
    else if ((nHashType & 0x1f) == SIGHASH_SINGLE)
    {
        ...
        if (nOut >= txTmp.vout.size())
        {
            if (fDebug)
            {
                LogPrint("script", "%s : ERROR: SignatureHash() : nOut=%d out of range \n", __FUNCTION__, nOut);
            }
...
bool SignSignature(const CKeyStore &keystore, const CScript& fromPubKey, CTransaction& txTo, unsigned int nIn, int nHashType)
{
    if (nIn >= txTo.vin.size())
    {
        if (fDebug)
        {
            LogPrint("script", "%s : ERROR - nIn >= txTo.vin.size() )\n", __FUNCTION__);
        }
...
bool SignSignature(const CKeyStore &keystore, const CTransaction& txFrom, CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    if (txin.prevout.n >= txFrom.vout.size())
    {
        if (fDebug)
        {
            LogPrint("script", "%s : ERROR - txin.prevout.n >= txFrom.vout.size() \n", __FUNCTION__);
        }
    ...
    for(const PAIRTYPE(txnouttype, CScript)& tplate: mTemplates)
    {
        ...
        while (true)
        {
            ...
            else if (opcode2 == OP_SMALLDATA)
            {
                ...
                if (opcode1 >= OP_PUSHDATA1
                    || vch1.size() > MAX_OP_RETURN_RELAY)
                {
                    break;
                }
            }
...
bool Solver(const CKeyStore& keystore, const CScript& scriptPubKey, uint256 hash, int nHashType, CScript& scriptSigRet, txnouttype& whichTypeRet)
{
    ...
    switch (whichTypeRet)
    {
        case TX_NONSTANDARD: case TX_NULL_DATA:
        {
            return false;
        }
        break;
...
int ScriptSigArgsExpected(txnouttype t, const std::vector<std::vector<unsigned char> >& vSolutions)
{
    switch (t)
    {
        ...
        case TX_SCRIPTHASH:
        {
            // doesn't include args needed by the script
            return 1;
        }
        break;
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash())
    {
        ...
        if (stackCopy.empty() == true)
        {
            if (fDebug)
            {
                LogPrint("script", "%s : ERROR - stackCopy.empty() = true \n", __FUNCTION__);
            }
...
static CScript CombineMultisig(CScript scriptPubKey, const CTransaction& txTo, unsigned int nIn, const vector<valtype>& vSolutions, vector<valtype>& sigs1, vector<valtype>& sigs2)
{
    ...
    if (vSolutions.size() <= 1)
    {
        if (fDebug)
        {
            LogPrint("script", "%s : ERROR - vSolutions.size() <= 1 \n", __FUNCTION__);
        }
...
static CScript CombineSignatures(CScript scriptPubKey, const CTransaction& txTo, unsigned int nIn, const txnouttype txType, const vector<valtype>& vSolutions, vector<valtype>& sigs1, vector<valtype>& sigs2)
{
    switch (txType)
    {
        ...
        case TX_PUBKEY: case TX_PUBKEYHASH:
        {
    ...
    switch(nSize)
    {
        ...
        case 0x04: case 0x05:
        {
            ...
            if (pubkey.size() != 65)
            {
                if (fDebug)
                {
                    LogPrint("script", "%s : ERROR - Pubkey.size() != 65 \n", __FUNCTION__);
                }
