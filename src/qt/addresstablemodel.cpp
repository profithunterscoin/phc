...
class AddressTablePriv
{
    public:
        ...
        void refreshAddressTable()
        {
            cachedAddressTable.clear();
            {
                ...
                BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& item, wallet->mapAddressBook)
                {
                    ...
                    cachedAddressTable.append(AddressTableEntry(fMine ? AddressTableEntry::Receiving : AddressTableEntry::Sending, QString::fromStdString(strName), QString::fromStdString(address.ToString())));
                }
                ...
                for (it = wallet->stealthAddresses.begin(); it != wallet->stealthAddresses.end(); ++it)
                {
                    ...
                    cachedAddressTable.append(AddressTableEntry(fMine ? AddressTableEntry::Receiving : AddressTableEntry::Sending, QString::fromStdString(it->label), QString::fromStdString(it->Encoded()), true));
                };
            }
        ...
        void updateEntry(const QString &address, const QString &label, bool isMine, int status)
        {
            ...
            QList<AddressTableEntry>::iterator lower = qLowerBound(cachedAddressTable.begin(), cachedAddressTable.end(), address, AddressTableEntryLessThan());
            QList<AddressTableEntry>::iterator upper = qUpperBound(cachedAddressTable.begin(), cachedAddressTable.end(), address, AddressTableEntryLessThan());
    ...
    if(role == Qt::EditRole)
    {
        switch(index.column())
        {
            case Label:
            {
                ...
                {
                    wallet->SetAddressBookName(CPHCcoinAddress(strTemp).Get(), value.toString().toStdString());
                }
            }
            break;
            ...
            case Address:
            {
                ...
                if(CPHCcoinAddress(rec->address.toStdString()) == CPHCcoinAddress(value.toString().toStdString()))
                {
                    editStatus = NO_CHANGES;
                ...
                else if(wallet->mapAddressBook.count(CPHCcoinAddress(value.toString().toStdString()).Get()))
                {
                ...
                else if(rec->type == AddressTableEntry::Sending)
                {
                    ...
                    // Global Namespace Start
                    {
                        LOCK(wallet->cs_wallet);
                        // Remove old entry
                        wallet->DelAddressBookName(CPHCcoinAddress(rec->address.toStdString()).Get());
                        // Add new entry with new address
                        wallet->SetAddressBookName(CPHCcoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                    }
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        ...
        else
        {
            ...
            // Check for duplicate addresses
            {
                ...
                if (wallet->mapAddressBook.count(CPHCcoinAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                ...
                wallet->SetAddressBookName(CPHCcoinAddress(strAddress).Get(), strLabel);
            }
        }
    }
    else if(type == Receive)
    {
        ...
        else
        {
            ...
            strAddress = CPHCcoinAddress(newKey.GetID()).ToString();
...
{
    {
        ...
        {
            CPHCcoinAddress address_parsed(sAddr);
