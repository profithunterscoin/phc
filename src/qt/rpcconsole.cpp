...
/**
 * Split PHC command line into a list of arguments. Aims to emulate \c bash and friends.
 *
...
RPCConsole::RPCConsole(QWidget *parent) : QWidget(parent), ui(new Ui::RPCConsole), historyPtr(0), cachedNodeid(-1), peersTableContextMenu(0), banTableContextMenu(0)
{
    ui->setupUi(this);
...
void RPCConsole::clear()
{
    ...
    {
        ui->messagesWidget->document()->addResource(QTextDocument::ImageResource, QUrl(ICON_MAPPING[i].url), QImage(ICON_MAPPING[i].source).scaled(ICON_SIZE, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
    }
