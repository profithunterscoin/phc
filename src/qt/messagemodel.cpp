...
class MessageTablePriv
{
    public:
        ...
        MessageTablePriv(MessageModel *parent): parent(parent) {}
            ...
            // Global Namespace Start
            {
                ...
                while (dbSmsg.NextSmesg(it, sPrefix, chKey, smsgStored))
                {
                    ...
                    if (SecureMsgDecrypt(false, smsgStored.sAddrTo, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
                    {
                        ...
                        addMessageEntry(MessageTableEntry(vchKey, MessageTableEntry::Received, label, QString::fromStdString(smsgStored.sAddrTo), QString::fromStdString(msg.sFromAddress), sent_datetime, received_datetime, (char*)&msg.vchMessage[0]), true);
                    }
                };
                ...
                while (dbSmsg.NextSmesg(it, sPrefix, chKey, smsgStored))
                {
                    ...
                    if (SecureMsgDecrypt(false, smsgStored.sAddrOutbox, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
                    {
                        ...
                        addMessageEntry(MessageTableEntry(vchKey, MessageTableEntry::Sent, label, QString::fromStdString(smsgStored.sAddrTo), QString::fromStdString(msg.sFromAddress), sent_datetime, received_datetime, (char*)&msg.vchMessage[0]), true);
                    }
                };
        ...
        void newMessage(const SecMsgStored& inboxHdr)
        {
            ...
            if (SecureMsgDecrypt(false, smsgStored.sAddrTo, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
            {
                ...
                addMessageEntry(MessageTableEntry(vchKey, MessageTableEntry::Received, label, QString::fromStdString(smsgStored.sAddrTo), QString::fromStdString(msg.sFromAddress), sent_datetime, received_datetime, (char*)&msg.vchMessage[0]), false);
            }
        }
        ...
        void newOutboxMessage(const SecMsgStored& outboxHdr)
        {
            ...
            if (SecureMsgDecrypt(false, smsgStored.sAddrOutbox, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
            {
                ...
                addMessageEntry(MessageTableEntry(vchKey, MessageTableEntry::Sent, label, QString::fromStdString(smsgStored.sAddrTo), QString::fromStdString(msg.sFromAddress), sent_datetime, received_datetime, (char*)&msg.vchMessage[0]), false);
            }
        }
    ...
    private:
        ...
        void addMessageEntry(const MessageTableEntry & message, const bool & append)
        {
            ...
            else
            {
                ...
                cachedMessageTable.insert(index, message);
...
MessageModel::MessageModel(CWallet *wallet, WalletModel *walletModel, QObject *parent) : QAbstractTableModel(parent), wallet(wallet), walletModel(walletModel), optionsModel(0), priv(0)
{
...
bool MessageModel::getAddressOrPubkey(QString &address, QString &pubkey) const
{
    ...
    if(addressParsed.IsValid())
    {
        ...
        // test if it's a local key
        if (SecureMsgGetStoredKey(destinationAddress, destinationKey) != 0 && SecureMsgGetLocalKey(destinationAddress, destinationKey) != 0)
        {
            return false;
        }
...
static void NotifySecMsgInbox(MessageModel *messageModel, SecMsgStored inboxHdr)
{
    ...
    QMetaObject::invokeMethod(messageModel, "newMessage", Qt::QueuedConnection, Q_ARG(SecMsgStored, inboxHdr));
}


static void NotifySecMsgOutbox(MessageModel *messageModel, SecMsgStored outboxHdr)
{
    QMetaObject::invokeMethod(messageModel, "newOutboxMessage", Qt::QueuedConnection, Q_ARG(SecMsgStored, outboxHdr));
}
