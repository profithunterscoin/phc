...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) : QObject(parent), wallet(wallet), fProcessingQueuedTransactions(false),
optionsModel(optionsModel), addressTableModel(0), transactionTableModel(0), cachedBalance(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0), cachedEncryptionStatus(Unencrypted), cachedNumBlocks(0)
{
...
void WalletModel::pollBalanceChanged()
{
    ...
    if(fForceCheckBalanceChanged
        || nBestHeight != cachedNumBlocks
        || nDarksendRounds != cachedDarksendRounds
...
void WalletModel::checkBalanceChanged()
{
        ...
        cachedTxLocks != nCompleteTXLocks || cachedWatchOnlyBalance != newWatchOnlyBalance || cachedWatchOnlyStake != newWatchOnlyStake || cachedWatchUnconfBalance != newWatchUnconfBalance || cachedWatchImmatureBalance != newWatchImmatureBalance)
    {
        ...
        emit balanceChanged(newBalance, newStake, newUnconfirmedBalance, newImmatureBalance, newAnonymizedBalance, newWatchOnlyBalance, newWatchOnlyStake, newWatchUnconfBalance, newWatchImmatureBalance);
    }
}
...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CCoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CCoinAddress(rcp.address.toStdString()).Get());
    ...
    // Global Namespace Start
    {
        ...
        if(!fCreated)
        {
            ...
            emit message(tr("Send Coins"), QString::fromStdString(strFailReason), false, CClientUIInterface::MSG_ERROR);
        ...
        if (nFeeRequired > insanefee)
        {
            if (fDebug)
            {
                LogPrint("wallet", "%s : OK - nFeeRequired: %d -- InsaneFee: %d\n", __FUNCTION__, nFeeRequired, insanefee);
            }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    // Global Namespace Start
    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.typeInd == AddressTableModel::AT_Stealth)
            {
                ...
                if (sxAddr.SetEncoded(sAddr))
                {
                    ...
                    if (GenerateRandomSecret(ephem_secret) != 0)
                    {
                        if (fDebug)
                        {
                            LogPrint("wallet", "%s : ERROR - GenerateRandomSecret failed. \n", __FUNCTION__);
                        }
                    ...
                    if (StealthSecret(ephem_secret, sxAddr.scan_pubkey, sxAddr.spend_pubkey, secretShared, pkSendTo) != 0)
                    {
                        if (fDebug)
                        {
                            LogPrint("wallet", "%s : ERROR - Could not generate receiving public key. \n", __FUNCTION__);
                        }
                    ...
                    if (!cpkTo.IsValid())
                    {
                        if (fDebug)
                        {
                            LogPrint("wallet", "%s : ERROR - Invalid public key generated. \n", __FUNCTION__);
                        }
                    ...
                    if (SecretToPublicKey(ephem_secret, ephem_pubkey) != 0)
                    {
                        if (fDebug)
                        {
                            LogPrint("wallet", "%s : ERROR - Could not generate ephem public key. \n", __FUNCTION__);
                        }
                    ...
                    if (rcp.narration.length() > 0)
                    {
                        ...
                        if (sNarr.length() > 24)
                        {
                            if (fDebug)
                            {
                                LogPrint("wallet", "%s : ERROR - Narration is too long. \n",  __FUNCTION__);
                            }
                        ...
                        if (!crypter.Encrypt((uint8_t*)&sNarr[0], sNarr.length(), vchNarr))
                        {
                            if (fDebug)
                            {
                                LogPrint("wallet", "%s : ERROR - Narration encryption failed. \n", __FUNCTION__);
                            }
                        ...
                        if (vchNarr.size() > 48)
                        {
                            if (fDebug)
                            {
                                LogPrint("wallet", "%s : ERROR - Encrypted narration is too long. \n", __FUNCTION__);
                            }
                ...
                else
                {
                    if (fDebug)
                    {
                        LogPrint("wallet", "%s : ERROR - Couldn't parse stealth address! \n", __FUNCTION__);
                    }
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CCoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
        ...
        if(!fCreated)
        {
            ...
            // FIXME: could cause collisions in the future
            if((total + nFeeRequired) > nBalance)
            {
                return SendCoinsReturn(AmountWithFeeExceedsBalance);
            }
...
static void NotifyAddressBookChanged(WalletModel *walletmodel, CWallet *wallet, const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
        QString strAddress = QString::fromStdString(CCoinAddress(address).ToString());
        QString strLabel = QString::fromStdString(label);

        qDebug() << "NotifyAddressBookChanged : "
                    + strAddress
                    + " "
                    + strLabel
                    + " isMine="
                    + QString::number(isMine)
                    + " status="
...
static void ShowProgress(WalletModel *walletmodel, const std::string &title, int nProgress)
{
    ...
    QMetaObject::invokeMethod(walletmodel, "showProgress", Qt::QueuedConnection, Q_ARG(QString, QString::fromStdString(title)), Q_ARG(int, nProgress));
    ...
    if (nProgress == 100)
    {
        ...
        // clear
        std::vector<std::pair<uint256, ChangeType> >().swap(vQueueNotifications);
    }
}
...
WalletModel::UnlockContext WalletModel::requestUnlock()
{
    ...
    if ((!was_locked)
        && fWalletUnlockStakingOnly && isAnonymizeOnlyUnlocked())
    {
...
WalletModel::UnlockContext::UnlockContext(WalletModel *wallet, bool valid, bool relock): wallet(wallet), valid(valid), relock(relock)
{
}
...
{
    // PHC context; old object no longer relocks wallet
    *this = rhs;
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for(const COutput& out: vCoins)
    {
        ...
        mapCoins[QString::fromStdString(CCoinAddress(address).ToString())].push_back(out);
    }
}
