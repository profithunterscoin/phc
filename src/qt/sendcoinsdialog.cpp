...
SendCoinsDialog::SendCoinsDialog(QWidget *parent) : QDialog(parent), ui(new Ui::SendCoinsDialog), clientModel(0), model(0), fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a PHC address (e.g. X8gZqgY4r2RoEdqYk3QsAqFckyf9pRHN6i)"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if(ui->checkUseDarksend->isChecked())
    {
        ...
        QString strNearestAmount(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), 0.1 * COIN));

        strFee = QString(tr("(darksend requires this amount to be rounded up to the nearest %1).").arg(strNearestAmount));
    }
    else
    {
...
void SendCoinsDialog::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchBalance, const CAmount& watchStake, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    Q_UNUSED(stake);
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    switch(sendCoinsReturn.status)
    {
        ...
        {
            msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 1000 PHC.");
            msgParams.second = CClientUIInterface::MSG_ERROR;
        }
        break;
        ...
        {
            QMessageBox::warning(this, tr("Send Coins"), tr("Error: The wallet was unlocked only to anonymize coins."), QMessageBox::Ok, QMessageBox::Ok);
        }
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid PHC address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
